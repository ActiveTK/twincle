...
- **倍精度 (`f64`)** を使用。
- コンソールと統合スクリプトは、単純和と `B2*` をともに小数点以下 17 桁で表示します。
- 現在の `f64` + Kahan 集計では、経験的には小数点以下 15 桁前後の一致が期待できます。
- 一方、理論上の保守的な保証桁は、出力される誤差上限に基づいて概ね小数点以下 14 桁程度です。
- したがって、表示桁数と保証桁数は同一ではありません。特に単純和の小数点以下 20 桁保証は、この実装では主張しません。
- `B2*` については、浮動小数点丸め誤差よりも Hardy-Littlewood 型の外挿誤差が支配的です。`B2*` を多くの桁で表示しても、それは Brun 定数に対する保証桁数を意味しません。

## ログとチェックポイント

- `run.log`: 実行ログ
- `exp_log_e*.jsonl`: `--exp` 指定時のチェックポイント記録
- `result_eX_part_Y.json`: `--exp X --splitmode --split Y` 指定時の split ログ
- `result_eX_summary.txt`: `python result_integrate.py` で生成される split 統合サマリ
...
このときログファイル名は `result_e17_part_2.json` のようになり、記録される `sum` は「全体の途中和」ではなく「その split 自体の部分和」です。

`result_eX_part_Y.json` の最後の `final` レコードには以下が入ります。
...
```bash
python result_integrate.py result_e17_part_1.json result_e17_part_2.json
```

統合スクリプトは JSONL を再生成しません。代わりに、画面表示と同じ内容をプレーンテキストで出力します。

- 既定の出力先は `result_eX_summary.txt`
- `--output` で任意のテキストファイル名を指定可能
- 画面には target、各 split の区間・双子素数個数・部分和・実行環境・速度・経過時間を整形表示
- 最後に、統合した Brun 部分和、`B2*`、誤差上限を表示

統合時の和の計算は Python 側で `Decimal` を使って行い、split JSON に書かれた十進値をそのまま厳密に合算します。  
したがって、統合処理そのものが追加の浮動小数点丸め誤差を持ち込みません。  
ただし `missed_sum` と `B2*` の補正項は、元の `f64` 系の値を取り込んで表示します。
